...
Для оценки сложности задачи был реализован простой CNN-классификатор (3 сверточных слоя + полносвязный классификатор) на тех же данных.  

- **Результат baseline**: Accuracy ~70%  
  - Показывает, что задача классификации "чистый / грязный" достаточно сложная для простых моделей.  
  - Позволяет обосновать использование более мощных моделей (например, EfficientNet) для достижения высокой точности.
